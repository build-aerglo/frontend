<template>
  ...
  >
    ...
    <div class="relative z-10 max-w-3xl font-sans px-4">
      <h1
        class="font-bold text-white drop-shadow-lg mb-0
    ...
    <!-- Pills + CTAs -->
    <div class="relative z-10 w-full flex flex-col items-center gap-5 mt-6 px-4">
  ...
  <section class="py-14 px-2">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-3 mb-8">
        ...
        <span class="text-2xl md:text-3xl font-bold text-slate-800">How It Works</span>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="absolute top-6 right-6 text-5xl font-black text-slate-100 select-none">
            {{ i + 1 }}
          </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col md:flex-row items-center gap-12">
        ...
        <div class="w-full md:w-1/2 md:order-2 relative flex justify-center items-center">
          <div class="absolute w-72 h-72 bg-primary/10 rounded-full blur-3xl animate-pulse"></div>
          ...
          >
            <div class="bg-slate-800 px-5 py-3 flex items-center gap-2">
              ...
              <div class="ml-3 text-xs text-slate-400 font-mono">Clereview · Business Dashboard</div>
            </div>
            ...
            <div class="p-5 space-y-4">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <div class="flex items-center gap-1">
                    <i v-for="n in 5" :key="n" class="pi pi-star-fill text-gold text-sm"></i>
                    <span class="ml-2 font-bold text-slate-800 text-lg">4.8</span>
                  </div>
                </div>
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <div class="flex-1 bg-slate-100 rounded-full h-2">
                    <div class="h-2 rounded-full !bg-primary" :style="`width:${bar.pct}%`"></div>
                  </div>
              ...
              <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                ...
                <div class="flex items-center gap-1 mb-1">
                  <i v-for="n in 5" :key="n" class="pi pi-star-fill text-gold text-[10px]"></i>
                </div>
                <p class="text-xs text-slate-600 italic">
                  "Absolutely seamless experience. I'll definitely be back!"
                </p>
                <div class="text-[10px] text-slate-400 mt-1">— Adaeze O., 2 hours ago</div>
              </div>
          ...
          >
            ...
            <span class="text-[10px] font-bold text-slate-700 uppercase tracking-wider">Live Insights</span>
          </div>
        </div>
        ...
        <div class="w-full md:w-1/2 md:order-1 space-y-8">
          <div>
            <div class="flex items-center gap-3 mb-3">
              <div class="w-1 h-8 rounded-full !bg-primary"></div>
              <span class="text-2xl md:text-3xl font-bold text-slate-800">Your Business Dashboard</span>
            </div>
            <p class="text-slate-500 text-base">
              Everything you need to manage your reputation and grow customer trust all in one clean dashboard.
            </p>
          </div>
  ...
  <section class="py-14 px-2">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-3 mb-8">
        ...
        <span class="text-2xl md:text-3xl font-bold text-slate-800">What Businesses Are Saying</span>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        >
          <div class="flex items-center gap-1 mb-3">
            <i v-for="n in 5" :key="n" class="pi pi-star-fill text-gold text-xs"></i>
          </div>
          <p class="text-slate-600 text-sm leading-relaxed italic mb-4">"{{ review.quote }}"</p>
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="font-semibold text-slate-800 text-sm">{{ review.name }}</div>
  ...
  <section class="py-14 px-2 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-3 mb-2">
        <div class="w-1 h-8 rounded-full !bg-primary"></div>
        <span class="text-2xl md:text-3xl font-bold text-slate-800">Industries We Cover</span>
      </div>
      <p class="text-slate-500 mb-10 pl-4">
        Whatever your business type, customers are already looking for you on Clereview.
      </p>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
        ...
        >
          ...
          <div class="font-semibold text-slate-700 text-sm leading-tight">{{ cat.label }}</div>
          <div class="text-xs text-slate-400">{{ cat.count }} businesses</div>
        </div>
      </div>
      <p class="text-center text-slate-400 text-sm mt-8">
        Don't see your industry?
        <NuxtLink to="/business/register" class="!text-primary font-semibold hover:underline ml-1">
  ...
  >
    <div class="flex-1">
      <div class="font-bold text-[130%] text-white">Ready to grow with Clereview?</div>
      <div class="text-white">
        Join hundreds of businesses using real customer feedback to build trust and win more customers.
      </div>
    </div>
...
onMounted((): void => {
  ...
  img.onload = (): void => { imageLoaded.value = true }
...
const statistics = [
  ...
  { title: 'Active Reviewers',      count: '100k+' },
  { title: 'Reviews Uploaded',      count: '10M+' },
  // { title: 'Avg Business Rating',   count: '4.8★' },
]

const categories = [
  { icon: 'pi-shop',         label: 'Retail & Shopping',      count: '40+' },
  { icon: 'pi-building',     label: 'Real Estate',            count: '25+' },
  { icon: 'pi-heart',        label: 'Health & Wellness',      count: '35+' },
  { icon: 'pi-car',          label: 'Automotive',             count: '18+' },
  { icon: 'pi-bolt',         label: 'Tech & Electronics',     count: '22+' },
  { icon: 'pi-home',         label: 'Home Services',          count: '30+' },
  { icon: 'pi-graduation-cap', label: 'Education & Training', count: '15+' },
  { icon: 'pi-utensils',     label: 'Food & Restaurants',     count: '50+' },
  { icon: 'pi-briefcase',    label: 'Professional Services',  count: '28+' },
  { icon: 'pi-palette',      label: 'Beauty & Lifestyle',     count: '32+' },
  { icon: 'pi-globe',        label: 'Travel & Hospitality',   count: '20+' },
  { icon: 'pi-plus-circle',  label: 'More Categories',        count: '60+' },
]
...
const features = [
  { icon: 'pi-star',       title: 'Review Management',      desc: 'Read, respond, and flag reviews from one place.' },
  { icon: 'pi-chart-line', title: 'Performance Analytics',  desc: 'Track rating trends, review volume, and customer sentiment.' },
  { icon: 'pi-bell',       title: 'Instant Notifications',  desc: 'Get alerted the moment a new review goes live.' },
  { icon: 'pi-globe',      title: 'Public Business Profile', desc: 'A beautiful profile page customers can find and trust.' },
  { icon: 'pi-shield',     title: 'Reputation Tools',       desc: 'Flag unfair reviews and protect your brand image.' },
]
...
const testimonials = [
  {
    quote: 'Since joining Clereview, our new customer enquiries went up 40%. Authentic reviews truly make a difference.',
    name: 'Emeka Obi',
    business: 'Obi Electronics, Lagos',
  },
  {
    quote: 'The dashboard makes it so easy to respond quickly. Our customers feel heard and keep coming back.',
    name: 'Fatima Bello',
    business: 'Bella Beauty Studio, Abuja',
  },
  {
    quote: 'Clereview helped us identify weak spots in our service. We fixed them and our rating jumped to 4.9.',
    name: 'Chidi Nwosu',
