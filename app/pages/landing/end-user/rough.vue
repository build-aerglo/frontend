<template>
  ...
  >
    <div class="container mx-auto px-6 flex items-center justify-between h-16">
      ...
      <!-- Desktop Nav Links -->
      <ul class="hidden lg:flex items-center space-x-8 text-gray-700 dark:text-white font-large">
    ...
    <!-- Fullscreen Sidebar for small screens -->
<div
  :class="[
    'fixed inset-0 bg-white dark:bg-gray-900 z-50 transform transition-transform duration-300 ease-in-out lg:hidden',
...
>
  ...
  <ul class="flex flex-col text-gray-700 dark:text-gray-200 font-medium p-6 space-y-6">
    ...
    <li>
      <NuxtLink
        to="/auth/end-user/sign-in"
        class="inline-flex items-center justify-center bg-[#008253] text-white px-4 py-3 rounded-lg hover:bg-primary/90 transition w-full text-center"
      >
        Login/Register
      </NuxtLink>
    </li>
...
<!-- Blur Overlay (covers everything when sidebar is open) -->
<div
  v-if="isOpen"
