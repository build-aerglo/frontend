<template>
    <button :type="type"
        :class="inputClass, rounded ? 'rounded-full' : 'rounded-[5px]', size && size === 'lg' ? 'py-[10px] px-[15px]' : 'py-[6px] px-[7px]', isLoading ? 'cursor-not-allowed' : 'cursor-pointer', primary ? '!bg-button text-white' : 'border-button border text-button bg-white'"
        class="flex items-center justify-center gap-[5px] text-[14px] w-full 
               transition transform duration-150 ease-in-out hover:scale-105"
        @click="$emit('clicked')"
        :disabled="isLoading ? 'disabled' : disabled">
    <i v-if="icon" :class="`pi pi-${icon}`"></i>
    {{ label }}
</button>
</template>

<script setup>
const props = defineProps(['type', 'primary', 'label', 'disabled', 'icon', 'isLoading', 'size', 'inputClass', 'rounded']);
const events = defineEmits(['clicked'])
</script>