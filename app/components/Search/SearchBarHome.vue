<template>
  ...
  <div class="w-full flex justify-center mt-1">
    <div
      ref="searchContainer"
      class="relative rounded-lg flex items-center bg-transparent dark:bg-gray-900 shadow-md border border-gray-300 dark:border-gray-700 focus-within:ring-2 focus-within:ring-[#008253] transition-all duration-300 z-[10]"
      :class="
        fullWidth ? 'w-full' : 'w-[85%] sm:max-w-sm md:max-w-md lg:max-w-lg'
      "
    >
      <input
        v-model="query"
        ...
        class="w-2/3 flex-1 h-12 sm:h-14 bg-transparent outline-none text-white dark:text-white rounded-l-lg placeholder-gray-400 px-4 py-4"
      />

      <button
        @click="handleSearchButton"
        :class="fullWidth ? 'w-[40px]' : 'w-1/3'"
        class="h-12 sm:h-14 px-1 bg-[#008253] text-white justify-center items-center rounded-r-lg hover:bg-[#006f45] transition-colors duration-200"
      >
        <i v-if="fullWidth || isSmallScreen" class="pi pi-search text-xs"></i>
      ...
      <ul
        v-if="showSuggestions"
        class="absolute top-full left-0 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.15)] z-[20] overflow-hidden"
      >
        <template v-if="hasSuggestions">
          <li
            class="px-4 py-2 text-xs font-bold uppercase tracking-wider text-gray-500 bg-gray-50 dark:bg-gray-700/50 dark:text-gray-400"
          >
            Companies
          </li>
          ...
          <li
            ...
            class="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
          >
            <div class="flex items-center space-x-3 min-w-0">
              <img
                :src="
                  item.logoUrl ||
                  'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=200&h=200&auto=format&fit=crop'
                "
              ...
              <div class="flex flex-col min-w-0">
                <span
                  class="text-gray-900 dark:text-gray-100 font-semibold truncate"
                >
                  {{ item.name }}
                </span>
        ...
        <li
          v-else
          class="px-4 py-8 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800"
        >
