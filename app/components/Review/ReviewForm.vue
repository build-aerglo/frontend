<template>
  <div class="space-y-4">
    ...
    <div ref="businessDropdownRef" class="relative">
      ...
      <div class="relative">
        <input 
          type="text" v-model="form.businessName" @input="handleBusinessInput"
          @focus="showSearchDropdown = true" placeholder="Search for a business..."
        ...
        <img 
          v-if="form.selectedBusinessId || form.isAddingNewBusiness" 
      ...
      <div v-if="form.businessName.trim() && !isSearching && filteredBusinesses.length === 0 && !form.selectedBusinessId && !form.isAddingNewBusiness" class="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
        <p class="text-sm text-amber-800 mb-2">Can't find "{{ form.businessName }}"?</p>
        <button @click="form.isAddingNewBusiness = true" class="text-sm bg-[#008253] text-white px-4 py-2 rounded-lg"> + Add as New Business </button>
      </div>
    ...
    <div v-if="form.isAddingNewBusiness" class="space-y-3 p-4 bg-green-50 rounded-lg border border-green-200">
      ...
      <select v-model="form.newBusinessCity" :disabled="!form.newBusinessState" class="w-full border rounded-lg px-3 py-2 text-sm outline-none disabled:bg-gray-100">
    ...
    <div v-if="form.selectedBusinessId && !form.isAddingNewBusiness" class="space-y-3">
      ...
      <select v-model="form.selectedBranchOption" :disabled="form.manualEntryEnabled || isLoadingBranches" class="w-full border rounded-lg px-3 py-2 text-sm outline-none disabled:bg-gray-100">
        <option value="">{{ isLoadingBranches ? 'Loading branches...' : 'Select a branch...' }}</option>
        <option value="online">Online (Digital Services)</option>
      ...
      <div class="flex items-center space-x-2">
        <input 
          type="checkbox" 
          id="formManual" 
          v-model="form.manualEntryEnabled" 
          :disabled="form.selectedBranchOption === 'online'"
          class="w-4 h-4 accent-[#008253]" 
        />
        <label for="formManual" class="text-sm text-gray-700">Branch not found? Enter manually</label>
      </div>

      <div v-if="form.manualEntryEnabled && form.selectedBranchOption !== 'online'" class="space-y-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <select v-model="form.manualCity" :disabled="!form.manualState" class="w-full border rounded-lg px-3 py-2 text-sm outline-none disabled:bg-gray-100">
          <option value="">Select City *</option>
      ...
      <p v-else-if="form.selectedBranchOption && !form.manualEntryEnabled" class="text-xs text-green-600 mt-1 font-medium">
        ✓ Existing branch selected
      </p>
      <p v-else-if="form.manualEntryEnabled && form.manualState && form.manualCity" class="text-xs text-green-600 mt-1 font-medium">
    ...
    <div>
      ...
      <div class="flex items-center">
        <Stars v-for="n in 5" :key="n" :value="(hoverRating || form.rating) - (n - 1)" :color-level="Math.ceil(hoverRating || form.rating)" class="cursor-pointer"
          @mousemove="hoverRating = n - 1 + getFraction($event)" @mouseleave="hoverRating = 0" @click="form.rating = n - 1 + getFraction($event)" />
    ...
    <div>
      ...
      <textarea v-model="form.reviewBody" rows="4" maxlength="500" class="w-full border rounded-lg px-3 py-2 resize-none focus:ring-2 focus:ring-[#008253] outline-none" placeholder="Tell us about your experience..."></textarea>
    ...
    <div class="pt-6 border-t">
      <div v-if="!userStore.isAuthenticated">
        <div v-if="!showEmailInput" class="grid grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-1">
            <button @click="emitOpenAuth" class="py-3 border border-[#008253] text-[#008253] rounded-xl font-medium hover:bg-green-100">Sign in and review</button>
        ...
        <div v-else>
          ...
          <input type="email" v-model="form.email" placeholder="Email is required for guest reviews" class="w-full border rounded-lg px-3 py-2 outline-none focus:ring-1 focus:ring-green-500 mb-3" />
          <div class="flex gap-2">
            <button @click="showEmailInput = false" class="flex-1 text-gray-500 underline text-sm">Back</button>
            <button @click="handleReviewSubmit" :disabled="isSubmitting" class="flex-[2] py-3 bg-[#008253] text-white rounded-xl">
              {{ isSubmitting ? 'Submitting...' : 'Submit Review' }}
            </button>
          </div>
        </div>
      </div>
      <button v-else @click="handleReviewSubmit" :disabled="isSubmitting" class="w-full py-4 bg-[#008253] text-white rounded-xl">
        {{ isSubmitting ? 'Submitting...' : 'Submit Review' }}
      </button>
    </div>
  </div>
...
const hasOnlineBranch = computed(() => branches.value.some(b => b.branchCityTown?.toLowerCase() === 'online'))

const branchOptions = computed(() => {
  return branches.value.map(branch => ({
    id: branch.id,
    display: [branch.branchCityTown, branch.branchState].filter(Boolean).join(', ') || 'Main Location'
  }))
})
...
const fetchBranches = async (id: string) => {
  ...
  try {
    ...
    if (profile?.data?.businessAddress) {
      all.push({ id: 'business-address', branchCityTown: 'Main', branchState: 'Location' })
    }
...
const handleReviewSubmit = async () => {
  ...
  try {
    const reviewData = {
      ...
      // isNewBranch is true for manual entries OR "online" selection
      isNewBranch: isManual || isOnline,
      
      // If new/online branch, locationId is null
      locationId: (isManual || isOnline) ? null : form.selectedBranchOption,
