...
export async function encryptJSONNative(data: unknown): Promise<string> {
  const config = useRuntimeConfig();
  const ENCRYPTION_KEY = config.public.local_encode_key as string;
...
export async function decryptJSONNative<T>(
